-- ═══════════════════════════════════════════════════════════
-- CRITICAL DATA INTEGRITY FIXES - FIXED VERSION
-- ═══════════════════════════════════════════════════════════
...
COMMENT ON COLUMN games.home_score IS 'Final score for home team';
COMMENT ON COLUMN games.away_score IS 'Final score for away team';

-- ───────────────────────────────────────────────────────────
-- 2. ADD API EVENT ID (Prevents Duplicate Games)
-- ───────────────────────────────────────────────────────────
ALTER TABLE games 
  ADD COLUMN IF NOT EXISTS api_event_id TEXT;

-- Add unique constraint if column already exists
DO $$ 
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'games_api_event_id_key'
  ) THEN
    ALTER TABLE games ADD CONSTRAINT games_api_event_id_key UNIQUE (api_event_id);
  END IF;
END $$;
...
-- ───────────────────────────────────────────────────────────
-- 4. ADD FULL TIMESTAMP (Better than separate date/time)
-- ───────────────────────────────────────────────────────────
ALTER TABLE games 
...
-- ───────────────────────────────────────────────────────────
-- 5. ADD STATUS TRANSITION TRACKING
-- ───────────────────────────────────────────────────────────
ALTER TABLE games 
...
-- ───────────────────────────────────────────────────────────
-- 7. IMPROVE ODDS HISTORY TRACKING
-- ───────────────────────────────────────────────────────────
...
-- Note: graded_at already exists in picks table

-- ───────────────────────────────────────────────────────────
-- 9. CREATE GAME STATUS HISTORY TABLE
-- ───────────────────────────────────────────────────────────
...
DROP POLICY IF EXISTS "Allow all access to game_status_history" ON game_status_history;
CREATE POLICY "Allow all access to game_status_history" ON game_status_history FOR ALL USING (true) WITH CHECK (true);

-- ───────────────────────────────────────────────────────────
-- 10. CREATE FUNCTION TO UPDATE GAME STATUS
-- ───────────────────────────────────────────────────────────
...
BEGIN
  ...
  IF v_old_status IS DISTINCT FROM p_new_status THEN
    ...
    UPDATE games 
    SET 
      status = p_new_status::game_status,
...
-- ───────────────────────────────────────────────────────────
-- 11. CREATE VIEW FOR GAMES WITH COMPUTED STATUS
-- ───────────────────────────────────────────────────────────
CREATE OR REPLACE VIEW games_with_computed_status AS
SELECT 
  g.*,
  CASE 
    WHEN g.game_start_timestamp IS NULL THEN g.status::TEXT
...
SELECT 
  ...
  COUNT(api_event_id) as games_with_api_id,
  COUNT(game_start_timestamp) as games_with_timestamp,
  COUNT(home_score) as games_with_scores
FROM games;

-- Expected output:
-- - total_games: your current game count
-- - games_with_api_id: 0 (will be populated on next ingestion)
-- - games_with_timestamp: should match total_games (populated from existing data)
-- - games_with_scores: number of completed games
