...
interface GameState {
  ...
  // Capper HP tracking (one HP bar per capper)
  capperHP: Map<string, { currentHP: number; maxHP: number }>; // Key: "gameId-side" (e.g., "game1-left")

  // Defense dots (protect HP - must be destroyed first)
  defenseDots: Map<string, DefenseDot>;
  projectiles: BaseProjectile[];

  // Actions
  initializeGame: () => void;
  initializeCapperHP: (gameId: string) => void;
  initializeDefenseDots: (gameId: string) => void;
  addProjectile: (projectile: BaseProjectile) => void;
  removeProjectile: (id: string) => void;
  applyDamageToCapperHP: (gameId: string, side: 'left' | 'right', damage: number) => void;
  applyDamage: (dotId: string, damage: number) => void;
  triggerFireOrb: (side: 'left' | 'right') => void;
