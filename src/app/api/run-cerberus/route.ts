...
export async function POST(request: Request) {
  ...
  try {
    runId = await startRunLog('cerberus', triggerType)
  } catch (logError) {
  ...
  try {
    ...
    const existingPicks = await getExistingPicksByGame('cerberus')
    ...
    console.log(`✅ Cerberus generated ${results.length} picks`)
    ...
    for (const result of results) {
      ...
      const { data: insertedPick, error: insertError } = await getSupabaseAdmin()
        ...
        .insert({
          ...
          algorithm_version: 'cerberus-v1-consensus',
          capper: 'cerberus',
    ...
    return NextResponse.json({
      success: true,
      message: `Cerberus generated ${results.length} picks`,
      picks: storedPicks,
      runId,
