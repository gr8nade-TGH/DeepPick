...
/**
 * IFRIT AUTO-PICKS CRON - TOTAL
 *
 * Runs every 7 minutes to automatically generate IFRIT TOTAL picks
 ...
 * - Each 7-minute cycle attempts ONLY ONE game
...
export async function GET(request: Request) {
  ...
  console.log(`\n${'='.repeat(80)}`)
  console.log(`🔥 [IFRIT-AUTO-PICKS] EXECUTION START: ${executionTime}`)
  console.log(`🔍 [IFRIT-AUTO-PICKS] REQUEST HEADERS:`, JSON.stringify({
  ...
  if (process.env.DISABLE_IFRIT_CRON === 'true') {
    console.log('🔥 [IFRIT-AUTO-PICKS] ⛔ KILL SWITCH ACTIVE - Cron is disabled via DISABLE_IFRIT_CRON env var')
  ...
  try {
    ...
    const lockKey = 'ifrit_nba_total_lock'
    const lockTimeout = 5 * 60 * 1000 // 5 minutes
    const now = new Date()
    const lockId = `cron_ifrit_nba_total_${Date.now()}_${Math.random().toString(36).substring(2, 8)}`

    console.log(`🔥 [IFRIT-AUTO-PICKS] Attempting to acquire lock: ${lockId}`)
    ...
    if (lockError) {
      console.error('🔥 [IFRIT-AUTO-PICKS] ❌ CRITICAL ERROR: Lock acquisition failed:', lockError)
      console.error('🔥 [IFRIT-AUTO-PICKS] ❌ This likely means the acquire_shiva_lock function does not exist!')
      console.error('🔥 [IFRIT-AUTO-PICKS] ❌ Falling back to manual lock check...')
      ...
      if (lockCheckError && lockCheckError.code !== 'PGRST116') {
        console.error('🔥 [IFRIT-AUTO-PICKS] ❌ CRITICAL ERROR: Lock check failed:', lockCheckError)
        return NextResponse.json({
          success: false,
      ...
      if (existingLock) {
        ...
        if (lockAge < lockTimeout) {
          console.log(`🔥 [IFRIT-AUTO-PICKS] ⚠️ SKIPPING - Another instance is running (locked ${Math.round(lockAge / 1000)}s ago by ${existingLock.locked_by})`)
          return NextResponse.json({
            success: false,
      ...
      if (upsertError) {
        console.error('🔥 [IFRIT-AUTO-PICKS] ❌ CRITICAL ERROR: Failed to acquire lock:', upsertError)
        return NextResponse.json({
          success: false,
      ...
      console.log(`🔥 [IFRIT-AUTO-PICKS] ⚠️ SKIPPING - Lock already held by another instance`)
      return NextResponse.json({
        success: false,
    ...
    console.log(`🔥 [IFRIT-AUTO-PICKS] ✅ Lock acquired: ${lockId}`)

    console.log('🔥 [IFRIT-AUTO-PICKS] Starting automated IFRIT pick generation...')
    const startTime = Date.now()

    // Step 1: Find eligible games (excludes games in cooldown or already picked)
    console.log('🎯 [IFRIT-AUTO-PICKS] Finding eligible games...')
    ...
    console.log(`🌐 [IFRIT-AUTO-PICKS] Using base URL: ${baseUrl}`)
    console.log(`🌐 [IFRIT-AUTO-PICKS] VERCEL_PROJECT_PRODUCTION_URL: ${process.env.VERCEL_PROJECT_PRODUCTION_URL}`)
    console.log(`🌐 [IFRIT-AUTO-PICKS] VERCEL_URL: ${process.env.VERCEL_URL}`)

    const scannerUrl = `${baseUrl}/api/shiva/step1-scanner`
    console.log(`🌐 [IFRIT-AUTO-PICKS] Calling scanner at: ${scannerUrl}`)
    ...
    console.log(`📡 [IFRIT-AUTO-PICKS] Scanner response status: ${scannerResponse.status}`)
    console.log(`📡 [IFRIT-AUTO-PICKS] Scanner response headers:`, Object.fromEntries(scannerResponse.headers.entries()))

    const responseText = await scannerResponse.text()
    console.log(`📡 [IFRIT-AUTO-PICKS] Scanner response body (first 500 chars):`, responseText.substring(0, 500))

    let scannerResult
    try {
      scannerResult = JSON.parse(responseText)
    } catch (parseError) {
      console.error('❌ [IFRIT-AUTO-PICKS] Failed to parse scanner response as JSON:', parseError)
      console.error('❌ [IFRIT-AUTO-PICKS] Response was:', responseText.substring(0, 1000))
      throw new Error(`Scanner returned non-JSON response: ${responseText.substring(0, 200)}`)
    }

    console.log('📊 [IFRIT-AUTO-PICKS] Scanner result:', scannerResult)

    if (!scannerResult.success || !scannerResult.selected_game) {
      console.log('⚠️ [IFRIT-AUTO-PICKS] No eligible games found')
      const duration = Date.now() - startTime
      console.log(`\n${'='.repeat(80)}`)
      console.log(`⚠️ [IFRIT-AUTO-PICKS] EXECUTION COMPLETE: No eligible games`)
    ...
    console.log(`🎮 [IFRIT-AUTO-PICKS] Selected game: ${selectedGame.away_team.name} @ ${selectedGame.home_team.name}`)

    // Step 2: Run full pick generation pipeline
    console.log('⚡ [IFRIT-AUTO-PICKS] Running pick generation...')
    ...
    console.log('📈 [IFRIT-AUTO-PICKS] Pick generation result:', pickResult)
    ...
    const releaseLock = async () => {
      try {
        ...
        if (error) {
          console.error('🔥 [IFRIT-AUTO-PICKS] Error releasing lock:', error)
        } else {
          console.log('🔥 [IFRIT-AUTO-PICKS] ✅ Lock released')
        }
      } catch (err) {
        console.error('🔥 [IFRIT-AUTO-PICKS] Error in releaseLock:', err)
      }
    }

    console.log(`\n${'='.repeat(80)}`)
    if (pickResult.decision === 'PICK' && pickResult.pick) {
      console.log(`✅ [IFRIT-AUTO-PICKS] EXECUTION COMPLETE: Pick generated`)
      console.log(`Duration: ${duration}ms`)
      console.log(`${'='.repeat(80)}\n`)
      ...
      return NextResponse.json({
        success: true,
        message: 'IFRIT pick generated successfully',
        picksGenerated: 1,
      ...
      console.log(`⚠️ [IFRIT-AUTO-PICKS] EXECUTION COMPLETE: Game resulted in PASS (cooldown recorded)`)
      console.log(`Duration: ${duration}ms`)
      console.log(`${'='.repeat(80)}\n`)
      ...
      console.log(`❌ [IFRIT-AUTO-PICKS] EXECUTION COMPLETE: Unexpected result`)
      console.log(`Duration: ${duration}ms`)
      console.log(`${'='.repeat(80)}\n`)
    ...
    console.error('❌ [IFRIT-AUTO-PICKS] Error:', error)
    ...
    try {
      ...
      await supabase
        .from('system_locks')
        .delete()
        .eq('lock_key', 'ifrit_nba_total_lock')
      console.log('🔥 [IFRIT-AUTO-PICKS] ✅ Lock released (error path)')
    } catch (lockErr) {
      console.error('🔥 [IFRIT-AUTO-PICKS] Error releasing lock on error:', lockErr)
    }
