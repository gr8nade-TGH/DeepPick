    ...
    <div className="min-h-screen bg-slate-950">
      ...
      <div className="container mx-auto px-4 py-4 max-w-[1600px]">
        ...
        {performance && (
          <div className="mb-4 grid grid-cols-5 gap-3">
            <div className="bg-slate-900/50 border border-slate-800 rounded px-3 py-2">
              <div className="text-xs text-slate-500 mb-0.5">Total Picks</div>
              <div className="text-lg font-semibold text-white">{performance.total_picks}</div>
            </div>
            <div className="bg-slate-900/50 border border-slate-800 rounded px-3 py-2">
              <div className="text-xs text-slate-500 mb-0.5">Win Rate</div>
              <div className="text-lg font-semibold text-emerald-400">{performance.win_rate.toFixed(1)}%</div>
            </div>
            <div className="bg-slate-900/50 border border-slate-800 rounded px-3 py-2">
              <div className="text-xs text-slate-500 mb-0.5">Wins / Losses</div>
              <div className="text-lg font-semibold text-white">{performance.wins} / {performance.losses}</div>
            </div>
            <div className="bg-slate-900/50 border border-slate-800 rounded px-3 py-2">
              <div className="text-xs text-slate-500 mb-0.5">ROI</div>
              <div className={`text-lg font-semibold ${performance.roi >= 0 ? 'text-emerald-400' : 'text-red-400'}`}>
                {performance.roi >= 0 ? '+' : ''}{performance.roi?.toFixed(1) || '0.0'}%
              </div>
            </div>
            <div className="bg-slate-900/50 border border-emerald-900/30 rounded px-3 py-2">
              <div className="text-xs text-slate-500 mb-0.5">Net Units</div>
              <div className={`text-lg font-semibold ${performance.net_units >= 0 ? 'text-emerald-400' : 'text-red-400'}`}>
        ...
        <div className="grid grid-cols-12 gap-4">
          ...
          {/* LEFT COLUMN - TODAY'S PICKS (60% width) */}
          <div className="col-span-7">
            <Card className="bg-slate-900/50 border-slate-800 h-full">
              <CardHeader className="pb-3 px-4 pt-3">
                <div className="flex items-center justify-between">
                  <CardTitle className="text-base font-semibold text-white">Today's Elite Picks</CardTitle>
                  <Tabs value={sportFilter} onValueChange={setSportFilter}>
                    <TabsList className="bg-slate-800/50 border border-slate-700 h-8">
                      ...
                      <TabsTrigger value="nba" className="text-xs px-2 py-1">NBA</TabsTrigger>
                      <TabsTrigger value="nfl" className="text-xs px-2 py-1">NFL</TabsTrigger>
                      <TabsTrigger value="mlb" className="text-xs px-2 py-1">MLB</TabsTrigger>
                    </TabsList>
                  </Tabs>
                </div>
              </CardHeader>
