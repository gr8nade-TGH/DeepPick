...
function formatSelectionAbbrev(selection: string): string {
  ...
  // Handle OVER/UNDER totals - keep as is (e.g., "OVER 232.5")
  ...
  if (spreadMatch) {
    ...
    // Try to find abbreviation in map, otherwise return as-is if already 3 chars
    const abbrev = NBA_TEAM_MAP[teamPart] || (teamPart.length <= 3 ? teamPart : teamPart)
    return `${abbrev} ${spread}`
  }

  // Handle moneyline - just team name
